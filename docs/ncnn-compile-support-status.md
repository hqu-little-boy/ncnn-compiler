...
`PP-OCRv6_tiny_rec`、`PP-OCRv6_small_rec`、`PP-OCRv6_small_rec_int8`、`PP-OCRv6_medium_rec`、`PP-OCRv6_medium_rec_int8`、`PP-OCRv6_tiny_det`、`PP-OCRv6_small_det`、
...
### 2.2 计算 op（35 个 source 计算层，36 个方言计算 op）

Importer 当前注册 36 个 source layer type，包含 `Input` 和下表 35 个计算层。方言另有一个
lowering 内部使用的 `ncnn.zero_point_cast`，因此共有 36 个计算 op。表中“支持”只表示对应受限实例可导入并通过产品
...
| `UnaryOp` | `ncnn.unary` | op_type | 仅支持 square（`op_type=4`）；YOLOv5 检测头 `wh=(2σ)²` 使用 |
| `Eltwise` | `ncnn.binary`（展开） | op_type, coeffs | 仅支持双输入 SUM；系数通过标量乘法展开为 BinaryOp |
...
| `Permute` | `ncnn.permute` | permutation | 支持 rank-2 order 0/1、rank-3 order 2/4，以及 rank-4 order 3（ncnn `d w h c`，YOLOv5 检测头 `[3,85,H,W]→[3,H,W,85]`） |
...
- CLI 接受 64 位 Linux ELF triple。当前宿主 Linux x86-64 的全新 Release 验收为 `374/374`
...
| 算子形态策略层 | `strategy-ncnn`（A1）：在向量化之前把卷积改写为投影映射的 matmul 形态——①1×1 s1 无条件 collapse 为 `[N·H·W,C]×[C,O]` 视图 matmul（动态空间维同样成立）；②其余 k×k 仅静态空间维且命中 ncnn `prefer_sgemm` 启发式（工作集字节 > L2 预算或任一通道 >16）时走 im2col gather + matmul；③conv 结果的唯一用户若为恒等逐元素 generic，则一并提升进折叠二维域（matmul → 2D generic → expand_shape），激活随 matmul 主循环落位。深度卷积 lower 为独立 op 不进本策略；Winograd（P7）落地为 `--conv-strategy=winograd` 的 F(6,3) 改写——3×3 s1 d1 且 IC>8/OC>8 的静态批 1 实例变换为 输入变换两段 generic + 编译期权重变换常量 [64,OC,IC] + 中心 `linalg.batch_matmul`（命中 P6-C 批量收缩内核，复用 P3 M×N 寄存器分块微内核）+ 输出变换两段 + tail tile 裁剪；`auto` 默认不启用（F(6,3) 改变累加结构，f32 相对误差 ~1e-3 量级，模型级启用须逐模型对账 golden 预算）。权重 collapse 由 canonicalizer 折叠为 `.rodata` 常量；累加顺序与直接卷积存在差异，由全量数值黄金测试按既定预算验收。CLI：`--conv-strategy={auto,gemm,conv,winograd}`、`--conv-gemm-l2-bytes=<N>`（默认 524288） |
...
| SIMD matmul 内核 | `matmul-kernel-ncnn`（A1b）：把 forall 内的静态 memref matmul 改写为显式向量 FMA 循环——M 外层每行将 C 读入寄存器累加器，K 内层零存储往返做 broadcast(A)·B 行累加，行末写回；行宽仅影响 LLVM 合法化拆分（宽至 1024 的 N 均覆盖）。同 pass 一并清理策略路径标量热点：恒等自拷贝循环删除、relu 型逐元 generic 行向量化（leading dims 发射 scf.parallel 保留多线程）、im2col gather（窗口映射纯转发拷贝）在通道数为 2 的幂时改写为 scf.parallel + 整 IC 行向量拷贝。PP 系大图模型的病态劣化已消除 |
...
- `models/yolov5n_test.cpp`：YOLOv5n 检测模型端到端；`3x640x640` 输入、`[25200,85]` 输出，
  与 upstream ncnn 对齐（`1e-4` 相对 + `2e-5` 绝对预算，固定输入下实测最大绝对误差约 `1.7e-3`
...
- `models/pp_lcnet_test.cpp`：PP-LCNet doc ori、textline ori、ChineseOCR Lite AngleNet、PP-OCRv6
  ...
  `1.853e-4`；PP-OCRv6 small rec 使用独立 `5e-4` 预算
...
- `models/dynamic_pp_ocr_test.cpp`：十六个 fixed-rank 动态产物：PP-LCNet doc/textline、AngleNet、
  PP-OCRv5 mobile/server rec、PP-OCRv6 tiny/small/medium rec、PP-OCRv6 small/medium rec INT8、
  PP-OCRv5 mobile/server det、PP-OCRv5 mobile det INT8 和 PP-OCRv6 tiny/small/medium det。覆盖多尺寸与交替尺寸、
...
- 静态数值基线通过 `ctest -L numerical-static-baseline` 独立执行，覆盖 35 个 source 计算层的静态
...
| 类别 | 限制 |
|---|---|
| 算子覆盖 | 35 个 source 计算层、36 个方言计算 op 的受限实例；DetectionOutput 仅 Caffe SSD；无 PriorBox/Proposal/Yolo、通用 group conv、RNN；UnaryOp 仅 square、Eltwise 仅双输入 SUM |
