...
| SIMD matmul 内核 | `matmul-kernel-ncnn`（A1b）：把 forall 内的静态 memref matmul 改写为显式向量 FMA 循环——M 外层每行将 C 读入寄存器累加器，K 内层零存储往返做 broadcast(A)·B 行累加，行末写回；行宽仅影响 LLVM 合法化拆分（宽至 1024 的 N 均覆盖）。同 pass 一并清理策略路径标量热点：恒等自拷贝循环删除、relu 型逐元 generic 行向量化、im2col gather（窗口映射纯转发拷贝）改写为整 IC 行向量拷贝。PP 系大图模型的病态劣化已消除 |
