...
| SIMD matmul 内核 | `matmul-kernel-ncnn`（A1b）：把 forall 内的静态 memref matmul 改写为显式向量 FMA 循环——M 外层每行将 C 读入寄存器累加器，K 内层零存储往返做 broadcast(A)·B 行累加，行末写回；N≤64 由 LLVM 合法化拆分到目标 ABI。消除策略层改写后通用标量展开在大图模型上的病态劣化。im2col gather 与 epilogue 逐元素循环仍为标量，属后续热点 |
