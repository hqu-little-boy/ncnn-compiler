...
- CLI 接受 64 位 Linux ELF triple。宿主 Linux x86-64 已通过完整 Release 构建、`216/216` CTest、
  数值模型和动态库执行验证。
...
| 量化 | 支持已覆盖的 INT8 scale-term 卷积/InnerProduct 重定标链；未覆盖的量化层仍由严格 lowering gate 拒绝；FP16/BF16 storage boundary 按精度策略处理 |
