...
TEST(LoadSqueezenetTyped, ImportsAndVerifies) {
  ...
  EXPECT_EQ(imported->get_operations().size(), 126u)
    << "total operation count is 126";
  EXPECT_EQ(imported->operation_count_of(OperationKind::Constant), 52u)
    << "constant operation count is 52";
  EXPECT_EQ(imported->get_operations().size() -
  ...
  EXPECT_TRUE(result_has_shape(conv1, kConv1Shape))
    << "conv1 shape is [64,113,113]";
  EXPECT_TRUE(result_has_shape(pool1, kPool1Shape))
    << "pool1 shape is [64,56,56]";
  EXPECT_TRUE(result_has_shape(pool3, kPool3Shape))
    << "pool3 shape is [128,28,28]";
  EXPECT_TRUE(result_has_shape(pool5, kPool5Shape))
    << "pool5 shape is [256,14,14]";
  EXPECT_TRUE(result_has_shape(conv10, kConv10Shape))
    << "conv10 K1 P1 shape is [1000,16,16]";
  EXPECT_TRUE(result_has_shape(pool10, kOutputShape))
