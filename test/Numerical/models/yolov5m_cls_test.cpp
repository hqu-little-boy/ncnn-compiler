...
const ReferenceModel kReference(
  YOLOV5M_CLS_PARAM_PATH, YOLOV5M_CLS_BIN_PATH, "in0", "out0", kInputShape);

TEST(NumericalModel, Yolov5MClsMatchesNcnn) {
  ...
  const std::vector<float> input =
    make_random_input(*input_elements, 0x59356d63U);
  const auto expected = run_ncnn_reference(kReference, input);
  ASSERT_TRUE(expected.has_value()) << expected.error();
  ASSERT_EQ(expected->size(), kOutputElements);

  CompiledModel compiled(YOLOV5M_CLS_LIBRARY_PATH, "yolov5m_cls");
