...
const ReferenceModel kReference(
  YOLOV5X_CLS_PARAM_PATH, YOLOV5X_CLS_BIN_PATH, "in0", "out0", kInputShape);

TEST(NumericalModel, Yolov5XClsMatchesNcnn) {
  ...
  const std::vector<float> input =
    make_random_input(*input_elements, 0x59357863U);
  const auto expected = run_ncnn_reference(kReference, input);
  ASSERT_TRUE(expected.has_value()) << expected.error();
  ASSERT_EQ(expected->size(), kOutputElements);

  CompiledModel compiled(YOLOV5X_CLS_LIBRARY_PATH, "yolov5x_cls");
