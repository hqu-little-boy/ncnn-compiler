...
TEST(NumericalDynamicModel,
     PPOCRv6TinyDetInt8StaticAndDynamicExecutionsAreConsistent) {
  CompiledModel static_compiled(PP_OCRV6_TINY_DET_INT8_STATIC_LIBRARY_PATH,
                                "pp_ocrv6_tiny_det_int8");
  CompiledModel compiled(PP_OCRV6_TINY_DET_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_tiny_det_int8_dynamic");
  CompiledModel infer(PP_OCRV6_TINY_DET_INT8_DYNAMIC_LIBRARY_PATH,
                      "pp_ocrv6_tiny_det_int8_dynamic_infer_output_shapes");
  ...
  for (const DetectionCase& shape : std::array<DetectionCase, 3>{
    ...
    const auto dimensions = input_shape(shape);
    std::array<std::int64_t, 3> inferred_shape{};
    ASSERT_EQ(infer.infer_dynamic(dimensions, inferred_shape), kSuccess);
    EXPECT_EQ(inferred_shape, expected_output_shape(shape));
...
TEST(NumericalDynamicModel, PPOCRv6TinyDetInt8SupportsAlternatingShapes) {
  CompiledModel compiled(PP_OCRV6_TINY_DET_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_tiny_det_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
  ...
  for (const DetectionCase& shape : std::array<DetectionCase, 4>{
    ...
    const auto dimensions = input_shape(shape);
    const std::vector<float> input(element_count(shape), 0.001F);
    std::vector<float> output(static_cast<std::size_t>(shape.height) *
                              static_cast<std::size_t>(shape.width));
    EXPECT_EQ(compiled.run_dynamic(input, dimensions, output, output.size()),
              kSuccess);
  }
...
TEST(NumericalDynamicModel, PPOCRv6TinyDetInt8RejectsInvalidShapesAndCapacity) {
  CompiledModel compiled(PP_OCRV6_TINY_DET_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_tiny_det_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
  ...
  for (const auto dimensions :
       std::array<std::array<std::int64_t, 3>, 6>{{{3, 31, 32},
                                                   {3, 32, 31},
                                                   {3, 33, 32},
                                                   {3, 320, 321},
                                                   {3, 0, 320},
...
TEST(NumericalDynamicModel,
     PPOCRv6TinyDetInt8ArtifactsDescribeQuantizedDynamicAbi) {
  EXPECT_GT(
    std::filesystem::file_size(PP_OCRV6_TINY_DET_INT8_DYNAMIC_LIBRARY_PATH),
    0U);
  const std::string manifest =
    read_text(PP_OCRV6_TINY_DET_INT8_DYNAMIC_MANIFEST_PATH);
  EXPECT_NE(manifest.find("pp_ocrv6_tiny_det_int8_dynamic"), std::string::npos);
  ...
  const std::string header =
    read_text(PP_OCRV6_TINY_DET_INT8_DYNAMIC_HEADER_PATH);
  EXPECT_NE(
    header.find("PP_OCRV6_TINY_DET_INT8_DYNAMIC_INPUT1_DYNAMIC_DIM_MASK "
                "UINT32_C(0x6)"),
    std::string::npos);
  EXPECT_NE(
    header.find("PP_OCRV6_TINY_DET_INT8_DYNAMIC_OUTPUT1_DYNAMIC_DIM_MASK "
                "UINT32_C(0x6)"),
    std::string::npos);
  EXPECT_NE(header.find("pp_ocrv6_tiny_det_int8_dynamic_infer_output_shapes"),
            std::string::npos);

  const std::string ncnn_ir =
    read_text(PP_OCRV6_TINY_DET_INT8_DYNAMIC_NCNN_IR_PATH);
  EXPECT_NE(ncnn_ir.find("int8_scale_term = 101"), std::string::npos);
  ...
  const std::string linalg_ir =
    read_text(PP_OCRV6_TINY_DET_INT8_DYNAMIC_LINALG_IR_PATH);
  EXPECT_NE(linalg_ir.find("linalg.conv_2d_nhwc_hwcf"), std::string::npos);
  EXPECT_NE(linalg_ir.find("linalg.depthwise_conv_2d_nhwc_hwcm"),
            std::string::npos);
...
TEST(NumericalDynamicModel,
     PPOCRv6SmallDetInt8StaticAndDynamicExecutionsAreConsistent) {
  CompiledModel static_compiled(PP_OCRV6_SMALL_DET_INT8_STATIC_LIBRARY_PATH,
                                "pp_ocrv6_small_det_int8");
  CompiledModel compiled(PP_OCRV6_SMALL_DET_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_small_det_int8_dynamic");
  CompiledModel infer(PP_OCRV6_SMALL_DET_INT8_DYNAMIC_LIBRARY_PATH,
                      "pp_ocrv6_small_det_int8_dynamic_infer_output_shapes");
  ASSERT_TRUE(static_compiled.valid()) << static_compiled.error();
  ASSERT_TRUE(compiled.valid()) << compiled.error();
  ASSERT_TRUE(infer.valid()) << infer.error();
...
TEST(NumericalDynamicModel, PPOCRv6SmallDetInt8SupportsAlternatingShapes) {
  CompiledModel compiled(PP_OCRV6_SMALL_DET_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_small_det_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
...
TEST(NumericalDynamicModel,
     PPOCRv6SmallDetInt8RejectsInvalidShapesAndCapacity) {
  CompiledModel compiled(PP_OCRV6_SMALL_DET_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_small_det_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
...
TEST(NumericalDynamicModel,
     PPOCRv6SmallDetInt8ArtifactsDescribeQuantizedDynamicAbi) {
  EXPECT_GT(
    std::filesystem::file_size(PP_OCRV6_SMALL_DET_INT8_DYNAMIC_LIBRARY_PATH),
    0U);
  const std::string manifest =
    read_text(PP_OCRV6_SMALL_DET_INT8_DYNAMIC_MANIFEST_PATH);
  EXPECT_NE(manifest.find("pp_ocrv6_small_det_int8_dynamic"),
            std::string::npos);
  EXPECT_NE(manifest.find("\"minimum\": 32"), std::string::npos);
  EXPECT_NE(manifest.find("\"multiple_of\": 32"), std::string::npos);
  EXPECT_NE(manifest.find("\"shape_program\""), std::string::npos);
  ...
  const std::string header =
    read_text(PP_OCRV6_SMALL_DET_INT8_DYNAMIC_HEADER_PATH);
  EXPECT_NE(
    header.find("PP_OCRV6_SMALL_DET_INT8_DYNAMIC_INPUT1_DYNAMIC_DIM_MASK "
                "UINT32_C(0x6)"),
    std::string::npos);
  EXPECT_NE(
    header.find("PP_OCRV6_SMALL_DET_INT8_DYNAMIC_OUTPUT1_DYNAMIC_DIM_MASK "
                "UINT32_C(0x6)"),
    std::string::npos);
  EXPECT_NE(header.find("pp_ocrv6_small_det_int8_dynamic_infer_output_shapes"),
            std::string::npos);

  const std::string ncnn_ir =
    read_text(PP_OCRV6_SMALL_DET_INT8_DYNAMIC_NCNN_IR_PATH);
  EXPECT_NE(ncnn_ir.find("int8_scale_term = 101"), std::string::npos);
  EXPECT_NE(ncnn_ir.find("int8_scale_term = 102"), std::string::npos);
  EXPECT_NE(ncnn_ir.find("tensor<96x1x7x7xi8>"), std::string::npos);
  const std::string linalg_ir =
    read_text(PP_OCRV6_SMALL_DET_INT8_DYNAMIC_LINALG_IR_PATH);
  EXPECT_NE(linalg_ir.find("linalg.conv_2d_nhwc_hwcf"), std::string::npos);
  EXPECT_NE(linalg_ir.find("linalg.depthwise_conv_2d_nhwc_hwcm"),
            std::string::npos);
  EXPECT_NE(linalg_ir.find("arith.fptosi"), std::string::npos);
...
TEST(NumericalDynamicModel,
     PPOCRv6MediumDetInt8StaticAndDynamicExecutionsAreConsistent) {
  CompiledModel static_compiled(PP_OCRV6_MEDIUM_DET_INT8_STATIC_LIBRARY_PATH,
                                "pp_ocrv6_medium_det_int8");
  CompiledModel compiled(PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_medium_det_int8_dynamic");
  CompiledModel infer(PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_LIBRARY_PATH,
                      "pp_ocrv6_medium_det_int8_dynamic_infer_output_shapes");
  ASSERT_TRUE(static_compiled.valid()) << static_compiled.error();
  ASSERT_TRUE(compiled.valid()) << compiled.error();
  ASSERT_TRUE(infer.valid()) << infer.error();
...
TEST(NumericalDynamicModel, PPOCRv6MediumDetInt8SupportsAlternatingShapes) {
  CompiledModel compiled(PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_medium_det_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
...
TEST(NumericalDynamicModel,
     PPOCRv6MediumDetInt8RejectsInvalidShapesAndCapacity) {
  CompiledModel compiled(PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_medium_det_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
...
TEST(NumericalDynamicModel,
     PPOCRv6MediumDetInt8ArtifactsDescribeQuantizedDynamicAbi) {
  EXPECT_GT(
    std::filesystem::file_size(PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_LIBRARY_PATH),
    0U);
  const std::string manifest =
    read_text(PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_MANIFEST_PATH);
  EXPECT_NE(manifest.find("pp_ocrv6_medium_det_int8_dynamic"),
            std::string::npos);
  EXPECT_NE(manifest.find("\"minimum\": 32"), std::string::npos);
  EXPECT_NE(manifest.find("\"multiple_of\": 32"), std::string::npos);
  EXPECT_NE(manifest.find("\"shape_program\""), std::string::npos);
  ...
  const std::string header =
    read_text(PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_HEADER_PATH);
  EXPECT_NE(
    header.find("PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_INPUT1_DYNAMIC_DIM_MASK "
                "UINT32_C(0x6)"),
    std::string::npos);
  EXPECT_NE(
    header.find("PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_OUTPUT1_DYNAMIC_DIM_MASK "
                "UINT32_C(0x6)"),
    std::string::npos);
  EXPECT_NE(header.find("pp_ocrv6_medium_det_int8_dynamic_infer_output_shapes"),
            std::string::npos);

  const std::string ncnn_ir =
    read_text(PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_NCNN_IR_PATH);
  EXPECT_NE(ncnn_ir.find("int8_scale_term = 101"), std::string::npos);
  EXPECT_NE(ncnn_ir.find("int8_scale_term = 102"), std::string::npos);
  EXPECT_NE(ncnn_ir.find("tensor<32x64x1x1xi8>"), std::string::npos);
  const std::string linalg_ir =
    read_text(PP_OCRV6_MEDIUM_DET_INT8_DYNAMIC_LINALG_IR_PATH);
  EXPECT_NE(linalg_ir.find("linalg.conv_2d_nhwc_hwcf"), std::string::npos);
  EXPECT_NE(linalg_ir.find("linalg.depthwise_conv_2d_nhwc_hwcm"),
            std::string::npos);
  EXPECT_NE(linalg_ir.find("arith.fptosi"), std::string::npos);
...
TEST(NumericalDynamicModel,
     PPLCNetTextlineOriInt8BackboneMatchesNcnnAcrossShapes) {
  CompiledModel compiled(PP_LCNET_TEXTLINE_ORI_INT8_BACKBONE_LIBRARY_PATH,
                         "pp_lcnet_x1_0_textline_ori_int8_backbone_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();

  for (const DetectionCase& shape : std::array<DetectionCase, 3>{
         DetectionCase{.height = 64, .width = 128},
         DetectionCase{.height = 80, .width = 160},
    ...
    const std::vector<float> input = make_random_input(
      element_count(shape),
      static_cast<std::uint32_t>(0x544C4242U + shape.height + shape.width),
      -1.0F,
      1.0F);
    const ReferenceModel reference(
      PP_LCNET_TEXTLINE_ORI_INT8_BACKBONE_PARAM_PATH,
      PP_LCNET_TEXTLINE_ORI_INT8_BACKBONE_BIN_PATH,
      "in0",
      "out0",
      TensorShape(
        static_cast<int>(shape.width), static_cast<int>(shape.height), 3));
...
TEST(NumericalDynamicModel,
     PPLCNetTextlineOriInt8StaticAndDynamicExecutionsAreConsistent) {
  CompiledModel static_compiled(PP_LCNET_TEXTLINE_ORI_INT8_STATIC_LIBRARY_PATH,
                                "pp_lcnet_x1_0_textline_ori_int8");
  CompiledModel dynamic_compiled(
    PP_LCNET_TEXTLINE_ORI_INT8_DYNAMIC_LIBRARY_PATH,
...
TEST(NumericalDynamicModel, PPLCNetTextlineOriInt8RejectsInvalidShapes) {
  CompiledModel compiled(PP_LCNET_TEXTLINE_ORI_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_lcnet_x1_0_textline_ori_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
  const std::array<std::int64_t, 3> normal_shape{3, 80, 160};
  const std::vector<float> input(3 * 80 * 160, 0.0F);
...
TEST(NumericalDynamicModel,
     PPLCNetTextlineOriInt8ArtifactsDescribeQuantizedAbi) {
  EXPECT_GT(
    std::filesystem::file_size(PP_LCNET_TEXTLINE_ORI_INT8_DYNAMIC_LIBRARY_PATH),
    0U);
  const std::string manifest =
    read_text(PP_LCNET_TEXTLINE_ORI_INT8_DYNAMIC_MANIFEST_PATH);
  EXPECT_NE(manifest.find("pp_lcnet_x1_0_textline_ori_int8_dynamic"),
            std::string::npos);
  EXPECT_NE(manifest.find("\"dynamic_dim_mask\": 6"), std::string::npos);
  const std::string header =
    read_text(PP_LCNET_TEXTLINE_ORI_INT8_DYNAMIC_HEADER_PATH);
  EXPECT_NE(header.find(
              "PP_LCNET_X1_0_TEXTLINE_ORI_INT8_DYNAMIC_INPUT1_DYNAMIC_DIM_MASK "
              "UINT32_C(0x6)"),
            std::string::npos);
  EXPECT_NE(header.find("PP_LCNET_X1_0_TEXTLINE_ORI_INT8_DYNAMIC_OUTPUT1_DIM0 "
                        "INT64_C(2)"),
            std::string::npos);
  const std::string ncnn_ir =
    read_text(PP_LCNET_TEXTLINE_ORI_INT8_DYNAMIC_NCNN_IR_PATH);
  EXPECT_NE(ncnn_ir.find("int8_scale_term = 1"), std::string::npos);
  EXPECT_NE(ncnn_ir.find("int8_scale_term = 102"), std::string::npos);
  EXPECT_NE(ncnn_ir.find("xi8>"), std::string::npos);
  const std::string linalg_ir =
    read_text(PP_LCNET_TEXTLINE_ORI_INT8_DYNAMIC_LINALG_IR_PATH);
  EXPECT_NE(linalg_ir.find("arith.fptosi"), std::string::npos);
  EXPECT_NE(linalg_ir.find("arith.sitofp"), std::string::npos);
  EXPECT_EQ(linalg_ir.find("ncnn.convolution"), std::string::npos);
}
...
TEST(NumericalDynamicModel, PPOCRv6TinyRecInt8InfersSequenceAcrossWidths) {
  CompiledModel infer(PP_OCRV6_TINY_REC_INT8_DYNAMIC_LIBRARY_PATH,
                      "pp_ocrv6_tiny_rec_int8_dynamic_infer_output_shapes");
  ASSERT_TRUE(infer.valid()) << infer.error();
...
TEST(NumericalDynamicModel,
     PPOCRv6TinyRecInt8StaticAndDynamicExecutionsAreConsistent) {
  CompiledModel staticCompiled(PP_OCRV6_TINY_REC_INT8_STATIC_LIBRARY_PATH,
                               "pp_ocrv6_tiny_rec_int8");
  CompiledModel compiled(PP_OCRV6_TINY_REC_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_tiny_rec_int8_dynamic");
  ASSERT_TRUE(staticCompiled.valid()) << staticCompiled.error();
  ASSERT_TRUE(compiled.valid()) << compiled.error();

  for (const std::int64_t width : std::array<std::int64_t, 3>{5, 64, 320}) {
    const std::array<std::int64_t, 3> dimensions = {3, 48, width};
    const std::vector<float> input =
      make_random_input(recognition_input_elements(width),
                        static_cast<std::uint32_t>(0x36544900U + width),
                        -1.0F,
                        1.0F);
    std::vector<float> actual(tiny_recognition_output_elements(width));
    ASSERT_EQ(compiled.run_dynamic(input, dimensions, actual, actual.size()),
              kSuccess)
      << "width " << width;
    for (std::int64_t row = 0; row < recognition_sequence_length(width);
      ...
      const auto values =
        std::span<const float>(actual).subspan(offset, kTinyRecClasses);
      EXPECT_TRUE(std::ranges::all_of(values,
                                      [](float value) {
        ...
        << "width " << width << ", row " << row;
      EXPECT_NEAR(
        std::accumulate(values.begin(), values.end(), 0.0F), 1.0F, 2.0e-4F)
        << "width " << width << ", row " << row;
    }
    if (width == 320) {
      std::vector<float> staticOutput(actual.size());
      ASSERT_EQ(staticCompiled.run(input, staticOutput), kSuccess);
      EXPECT_TRUE(compare_values(actual, staticOutput, 1.0e-5F));
    }
  }
  record_peak_rss();
}

TEST(NumericalDynamicModel, PPOCRv6TinyRecInt8SupportsAlternatingWidths) {
  CompiledModel compiled(PP_OCRV6_TINY_REC_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_tiny_rec_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
...
TEST(NumericalDynamicModel, PPOCRv6TinyRecInt8RejectsInvalidShapesAndCapacity) {
  CompiledModel compiled(PP_OCRV6_TINY_REC_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_tiny_rec_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
...
TEST(NumericalDynamicModel,
     PPOCRv6TinyRecInt8ArtifactsDescribeQuantizedDynamicAbi) {
  EXPECT_GT(
    std::filesystem::file_size(PP_OCRV6_TINY_REC_INT8_DYNAMIC_LIBRARY_PATH),
    0U);
  const std::string manifest =
    read_text(PP_OCRV6_TINY_REC_INT8_DYNAMIC_MANIFEST_PATH);
  EXPECT_NE(manifest.find("pp_ocrv6_tiny_rec_int8_dynamic"), std::string::npos);
  EXPECT_NE(manifest.find("\"minimum\": 5"), std::string::npos);
  EXPECT_NE(manifest.find("\"shape_program\""), std::string::npos);

  const std::string header =
    read_text(PP_OCRV6_TINY_REC_INT8_DYNAMIC_HEADER_PATH);
  EXPECT_NE(
    header.find("PP_OCRV6_TINY_REC_INT8_DYNAMIC_INPUT1_DYNAMIC_DIM_MASK "
                "UINT32_C(0x4)"),
    std::string::npos);
  EXPECT_NE(
    header.find("PP_OCRV6_TINY_REC_INT8_DYNAMIC_OUTPUT1_DIM1 INT64_C(6906)"),
    std::string::npos);
  EXPECT_NE(header.find("pp_ocrv6_tiny_rec_int8_dynamic_infer_output_shapes"),
            std::string::npos);

  const std::string ncnnIr =
    read_text(PP_OCRV6_TINY_REC_INT8_DYNAMIC_NCNN_IR_PATH);
  EXPECT_NE(ncnnIr.find("int8_scale_term = 101"), std::string::npos);
  EXPECT_NE(ncnnIr.find("int8_scale_term = 102"), std::string::npos);
  EXPECT_NE(ncnnIr.find("xi8>"), std::string::npos);
  const std::string linalgIr =
    read_text(PP_OCRV6_TINY_REC_INT8_DYNAMIC_LINALG_IR_PATH);
  EXPECT_NE(linalgIr.find("linalg.conv_2d_nhwc_hwcf"), std::string::npos);
  EXPECT_NE(linalgIr.find("linalg.depthwise_conv_2d_nhwc_hwcm"),
            std::string::npos);
  EXPECT_NE(linalgIr.find("arith.fptosi"), std::string::npos);
  EXPECT_NE(linalgIr.find("arith.sitofp"), std::string::npos);
  EXPECT_EQ(linalgIr.find("ncnn.convolution"), std::string::npos);
}
...
TEST(NumericalDynamicModel, PPOCRv5MobileRecInt8InfersSequenceAcrossWidths) {
  CompiledModel infer(PP_OCRV5_MOBILE_REC_INT8_DYNAMIC_LIBRARY_PATH,
                      "pp_ocrv5_mobile_rec_int8_dynamic_infer_output_shapes");
  ASSERT_TRUE(infer.valid()) << infer.error();
...
TEST(NumericalDynamicModel,
     PPOCRv5MobileRecInt8StaticAndDynamicExecutionsAreConsistent) {
  CompiledModel static_compiled(PP_OCRV5_MOBILE_REC_INT8_STATIC_LIBRARY_PATH,
                                "pp_ocrv5_mobile_rec_int8");
  CompiledModel compiled(PP_OCRV5_MOBILE_REC_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv5_mobile_rec_int8_dynamic");
  ASSERT_TRUE(static_compiled.valid()) << static_compiled.error();
  ASSERT_TRUE(compiled.valid()) << compiled.error();
  ...
  for (const std::int64_t width : std::array<std::int64_t, 3>{5, 64, 320}) {
    ...
    const std::vector<float> input =
      make_random_input(recognition_input_elements(width),
                        static_cast<std::uint32_t>(0x354D4938U + width),
                        -1.0F,
                        1.0F);
    std::vector<float> actual(recognition_output_elements(width));
    ASSERT_EQ(compiled.run_dynamic(input, dimensions, actual, actual.size()),
              kSuccess)
      << "width " << width;
    for (std::int64_t row = 0; row < recognition_sequence_length(width);
      ...
      EXPECT_TRUE(std::ranges::all_of(values,
        ...
        << "width " << width << ", row " << row;
      EXPECT_NEAR(
        std::accumulate(values.begin(), values.end(), 0.0F), 1.0F, 3.0e-4F)
        << "width " << width << ", row " << row;
    }
    if (width == 320) {
      std::vector<float> static_output(actual.size());
...
TEST(NumericalDynamicModel, PPOCRv5MobileRecInt8SupportsAlternatingWidths) {
  CompiledModel compiled(PP_OCRV5_MOBILE_REC_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv5_mobile_rec_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
...
TEST(NumericalDynamicModel,
     PPOCRv5MobileRecInt8RejectsInvalidShapesAndCapacity) {
  CompiledModel compiled(PP_OCRV5_MOBILE_REC_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv5_mobile_rec_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
...
TEST(NumericalDynamicModel,
     PPOCRv5MobileRecInt8ArtifactsDescribeQuantizedDynamicAbi) {
  EXPECT_GT(
    std::filesystem::file_size(PP_OCRV5_MOBILE_REC_INT8_DYNAMIC_LIBRARY_PATH),
    0U);
  const std::string manifest =
    read_text(PP_OCRV5_MOBILE_REC_INT8_DYNAMIC_MANIFEST_PATH);
  EXPECT_NE(manifest.find("pp_ocrv5_mobile_rec_int8_dynamic"),
            std::string::npos);
  EXPECT_NE(manifest.find("18385"), std::string::npos);
  EXPECT_NE(manifest.find("\"minimum\": 5"), std::string::npos);
  EXPECT_NE(manifest.find("\"shape_program\""), std::string::npos);
  ...
  const std::string header =
    read_text(PP_OCRV5_MOBILE_REC_INT8_DYNAMIC_HEADER_PATH);
  for (const std::string_view required : {
         "#define PP_OCRV5_MOBILE_REC_INT8_DYNAMIC_INPUT1_DYNAMIC_DIM_MASK "
         "UINT32_C(0x4)",
         "#define PP_OCRV5_MOBILE_REC_INT8_DYNAMIC_OUTPUT1_DIM1 INT64_C(18385)",
         "pp_ocrv5_mobile_rec_int8_dynamic_infer_output_shapes",
       }) {
    EXPECT_NE(header.find(required), std::string::npos) << required;
  }
...
TEST(NumericalDynamicModel,
     PPOCRv6MediumRecInt8InfersAndMatchesStaticExecution) {
  CompiledModel static_compiled(PP_OCRV6_MEDIUM_REC_INT8_STATIC_LIBRARY_PATH,
                                "pp_ocrv6_medium_rec_int8");
  CompiledModel compiled(PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_medium_rec_int8_dynamic");
  CompiledModel infer(PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_LIBRARY_PATH,
                      "pp_ocrv6_medium_rec_int8_dynamic_infer_output_shapes");
  ASSERT_TRUE(static_compiled.valid()) << static_compiled.error();
  ASSERT_TRUE(compiled.valid()) << compiled.error();
  ASSERT_TRUE(infer.valid()) << infer.error();
  for (const std::int64_t width : std::array<std::int64_t, 4>{5, 13, 64, 320}) {
    ...
    const std::vector<float> input =
      make_random_input(recognition_input_elements(width),
                        static_cast<std::uint32_t>(0x364D4938U + width),
                        -1.0F,
                        1.0F);
    std::vector<float> actual(medium_recognition_output_elements(width));
    ASSERT_EQ(compiled.run_dynamic(input, dimensions, actual, actual.size()),
    ...
    for (std::int64_t row = 0; row < recognition_sequence_length(width);
         ++row) {
      const auto values = std::span<const float>(actual).subspan(
        static_cast<std::size_t>(row) * kMediumRecClasses, kMediumRecClasses);
      EXPECT_TRUE(std::ranges::all_of(values, [](float value) {
        return std::isfinite(value) && value >= 0.0F && value <= 1.0F;
      }));
      EXPECT_NEAR(
...
TEST(NumericalDynamicModel, PPOCRv6MediumRecInt8SupportsAlternatingWidths) {
  CompiledModel compiled(PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_medium_rec_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
  for (const std::int64_t width : std::array<std::int64_t, 4>{320, 5, 64, 13}) {
...
TEST(NumericalDynamicModel,
     PPOCRv6MediumRecInt8RejectsInvalidShapesAndCapacity) {
  CompiledModel compiled(PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_LIBRARY_PATH,
                         "pp_ocrv6_medium_rec_int8_dynamic");
  ASSERT_TRUE(compiled.valid()) << compiled.error();
  const std::array<std::int64_t, 3> normal_shape = {3, 48, 5};
...
TEST(NumericalDynamicModel, PPOCRv6MediumRecInt8ArtifactsDescribeDynamicAbi) {
  EXPECT_GT(
    std::filesystem::file_size(PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_LIBRARY_PATH),
    0U);
  const std::string manifest =
    read_text(PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_MANIFEST_PATH);
  EXPECT_NE(manifest.find("pp_ocrv6_medium_rec_int8_dynamic"),
            std::string::npos);
  EXPECT_NE(manifest.find("18710"), std::string::npos);
  EXPECT_NE(manifest.find("\"minimum\": 5"), std::string::npos);
  const std::string header =
    read_text(PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_HEADER_PATH);
  EXPECT_NE(header.find(
              "PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_INPUT1_DIM2 NCNN_DYNAMIC_DIM"),
            std::string::npos);
  EXPECT_NE(
    header.find("PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_OUTPUT1_DIM1 INT64_C(18710)"),
    std::string::npos);
  const std::string ncnn_ir =
    read_text(PP_OCRV6_MEDIUM_REC_INT8_DYNAMIC_NCNN_IR_PATH);
  EXPECT_NE(ncnn_ir.find("int8_scale_term = 102"), std::string::npos);
  const std::string linalg_ir =
