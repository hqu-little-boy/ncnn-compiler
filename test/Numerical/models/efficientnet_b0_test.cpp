...
const ReferenceModel kReference(EFFICIENTNET_B0_PARAM_PATH,
                                EFFICIENTNET_B0_BIN_PATH,
                                "in0",
                                "out0",
                                kInputShape);

TEST(NumericalModel, EfficientNetB0MatchesNcnn) {
  ...
  const std::vector<float> input =
    make_random_input(*input_elements, 0x454e4230U);
  const auto expected = run_ncnn_reference(kReference, input);
  ASSERT_TRUE(expected.has_value()) << expected.error();
  ASSERT_EQ(expected->size(), kOutputElements);

  CompiledModel compiled(EFFICIENTNET_B0_LIBRARY_PATH, "efficientnet_b0");
