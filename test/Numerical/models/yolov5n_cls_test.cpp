...
const ReferenceModel kReference(
  YOLOV5N_CLS_PARAM_PATH, YOLOV5N_CLS_BIN_PATH, "in0", "out0", kInputShape);

TEST(NumericalModel, Yolov5NClsMatchesNcnn) {
  ...
  const std::vector<float> input =
    make_random_input(*input_elements, 0x59356e63U);
  const auto expected = run_ncnn_reference(kReference, input);
  ASSERT_TRUE(expected.has_value()) << expected.error();
  ASSERT_EQ(expected->size(), kOutputElements);

  CompiledModel compiled(YOLOV5N_CLS_LIBRARY_PATH, "yolov5n_cls");
