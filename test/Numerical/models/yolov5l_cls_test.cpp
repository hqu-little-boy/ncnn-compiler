...
const ReferenceModel kReference(
  YOLOV5L_CLS_PARAM_PATH, YOLOV5L_CLS_BIN_PATH, "in0", "out0", kInputShape);

TEST(NumericalModel, Yolov5LClsMatchesNcnn) {
  ...
  const std::vector<float> input =
    make_random_input(*input_elements, 0x59356c63U);
  const auto expected = run_ncnn_reference(kReference, input);
  ASSERT_TRUE(expected.has_value()) << expected.error();
  ASSERT_EQ(expected->size(), kOutputElements);

  CompiledModel compiled(YOLOV5L_CLS_LIBRARY_PATH, "yolov5l_cls");
