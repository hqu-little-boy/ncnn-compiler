...
TEST(NumericalModel, Yolov5nSegMatchesNcnn) {
  ...
  const std::vector<float> input =
    make_random_input(*input_elements, 0x53454731U);
  ...
  const auto expected = run_ncnn_reference(
    YOLOV5N_SEG_PARAM_PATH, YOLOV5N_SEG_BIN_PATH, kInputs, kOutputs);
  ASSERT_TRUE(expected.has_value()) << expected.error();
  ASSERT_EQ(expected->size(), 2U);
  ASSERT_EQ(expected->front().size(), kProtoElements);
  ASSERT_EQ(expected->back().size(), kDetectionElements);

  CompiledModel compiled(YOLOV5N_SEG_LIBRARY_PATH, "yolov5n_seg");
