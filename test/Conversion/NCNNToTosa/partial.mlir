...
// RUN: ncnn-mlir-opt --convert-ncnn-to-tosa --mlir-print-debuginfo --mlir-print-local-scope %s | FileCheck %s --check-prefix=LOCCHECK

func.func @partial(%arg0: tensor<2x3x4xf32>) -> tensor<2x3x4xf32> {
  %before = ncnn.relu %arg0 {ncnn.name = "before", ncnn.source_layer = 1 : i64} : (tensor<2x3x4xf32>) -> tensor<2x3x4xf32> loc("before"("ncnn-layer":1:0))
...
// The source-layer location carrier survives NCNNToTosa on the emitted op.
// LOCCHECK: tosa.clamp
// LOCCHECK: loc("before"("ncnn-layer":1:0))
