...
// 把解析好的 ncnn 计算图提升为 MLIR 模块：一个 func.func @model，输入是 ncnn
// Input 层（block argument），权重是 arith.constant，计算层是 ncnn 方言算子。
// 失败时返回带 layer 上下文的 ImportError；形状/类型推断的诊断信息会被捕获进
// message。
